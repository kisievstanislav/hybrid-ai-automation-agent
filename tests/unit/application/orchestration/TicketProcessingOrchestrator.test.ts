...
import type { ProcessingResultRecorder } from '../../../../src/application/ports/reporting/ProcessingResultRecorder.js';
...
import { CustomerType, QueueItemStatus, TicketStatus } from '../../../../src/domain/index.js';
...
describe('TicketProcessingOrchestrator', () => {
  it('should update an approved ticket and return completed', async () => {
    ...
    const processingResultRecorder: ProcessingResultRecorder = {
    ...
    expect(processingResultRecorder.record).toHaveBeenCalledOnce();
  });
  ...
  it('should throw when the ticket API fails', async () => {
    ...
    expect(ticketUiService.updateTicket).not.toHaveBeenCalled();
    expect(processingResultRecorder.record).not.toHaveBeenCalled();
  });
});
